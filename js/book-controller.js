...
function booksToDisplay() {
    if (!getBooks().length) {
        document.querySelector('.no-books-modal').innerHTML = 'There are no books avalible';
...
function onAddBook(event) {
    ...
    if (title === '' || price === '') showAlert('inValidInputs');
        ...
        addBook(title, price);
        showAlert('Book Added Successfuly', 'success');
        onClearFields();
        booksToDisplay();
    }
}
...
function onDeleteBook(bookId) {
    ...
    deleteBook(bookId);
    showAlert('Deleted Successfuly', 'danger');
    booksToDisplay();
}
